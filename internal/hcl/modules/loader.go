...
func NewModuleLoader(cachePath string, hclParser *SharedHCLParser, credentialsSource *CredentialsSource, sourceMap config.TerraformSourceMap, logger *logrus.Entry, moduleSync *intSync.KeyMutex) *ModuleLoader {
...
func (m *ModuleLoader) loadModule(moduleCall *tfconfig.ModuleCall, parentPath string, prefix string) (*ManifestModule, error) {
	...
	if err == nil {
		...
		m.logger.Debugf("module %s cannot be loaded, re-loading: %s", key, loadModErr)
	} else {
...
func (m *ModuleLoader) loadModuleFromPath(fullPath string) (*tfconfig.Module, error) {
	...
	for _, info := range fileInfos {
		...
		if fileDiag != nil && fileDiag.HasErrors() {
			return nil, fmt.Errorf("failed to parse file %s diag: %w", path, fileDiag.Error())
		}
		...
		if contentDiag != nil && contentDiag.HasErrors() {
			return nil, fmt.Errorf("failed to load module from file %s diag: %w", path, contentDiag.Error())
		}
	}
