...
func TestSensitive(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			cty.NumberIntVal(1).Mark(MarkedSensitive),
			``,
		},
		{
		...
		},
		{
			...
			cty.ListVal([]cty.Value{cty.NumberIntVal(1).Mark(MarkedSensitive)}),
			``,
		},
	}
	...
	for _, test := range tests {
		t.Run(fmt.Sprintf("sensitive(%#v)", test.Input), func(t *testing.T) {
			...
			if !got.HasMark(MarkedSensitive) {
...
func TestNonsensitive(t *testing.T) {
	tests := []struct {
		...
		{
			cty.DynamicVal.Mark(MarkedSensitive),
			``,
		},
		{
			cty.UnknownVal(cty.String).Mark(MarkedSensitive),
			``,
		},
		{
			cty.NullVal(cty.EmptyObject).Mark(MarkedSensitive),
			``,
		},
		{
			// The inner sensitive remains afterwards
			cty.ListVal([]cty.Value{cty.NumberIntVal(1).Mark(MarkedSensitive)}).Mark(MarkedSensitive),
			``,
		},
