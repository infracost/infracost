...
func TestS3BucketStandardStorageClass(t *testing.T) {
	stub := stubAWS(t)
	defer stub.Close()
	...
	for metric, sum := range requestSums {
		stub.WhenBody(fmt.Sprintf("MetricName=%s", metric), "Statistics.member.1=Sum", "Unit=Count").Then(200, fmt.Sprintf(`
		<GetMetricStatisticsResponse xmlns="http://monitoring.amazonaws.com/doc/2010-08-01/">
			<GetMetricStatisticsResult>
				<Label>%s</Label>
				<Datapoints>
		...
		</GetMetricStatisticsResponse>`, metric, sum))
	}
	...
	for metric, bytes := range dataBytes {
		stub.WhenBody(fmt.Sprintf("MetricName=%s", metric), "Statistics.member.1=Sum", "Unit=Bytes").Then(200, fmt.Sprintf(`
		<GetMetricStatisticsResponse xmlns="http://monitoring.amazonaws.com/doc/2010-08-01/">
			<GetMetricStatisticsResult>
				<Label>%s</Label>
				<Datapoints>
