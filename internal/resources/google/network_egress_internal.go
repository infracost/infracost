...
var NetworkEgressUsageSchema = []*schema.UsageItem{
...
func (r *NetworkEgressUsage) networkEgress(region string, resourceName, prefixName string, egressResourceType EgressResourceType) *schema.Resource {
	...
	// Same continent
	if r.doesEgressIncludeSameContinent(egressResourceType) {
	...
	regionsData := r.getEgressRegionsData(prefixName, egressResourceType)
	usageFiltersData := r.getEgressUsageFiltersData(egressResourceType)
	...
	for _, regData := range regionsData {
		...
		for idx, usageFilter := range usageFiltersData {
			...
			if regData.fixedRegion != "" {
				...
				apiRegion = r.getEgressAPIRegionName(region, egressResourceType)
			}
			var name string
			if usageName != "" {
			...
			resource.CostComponents = append(resource.CostComponents, &schema.CostComponent{
				...
				ProductFilter: &schema.ProductFilter{
					Region:           apiRegion,
					VendorName:       strPtr("gcp"),
					Service:          r.getEgressAPIServiceName(egressResourceType),
...
func (r *NetworkEgressUsage) doesEgressIncludeSameContinent(egressResourceType EgressResourceType) bool {
...
func (r *NetworkEgressUsage) getEgressUsageFiltersData(egressResourceType EgressResourceType) []*egressRegionUsageFilterData {
	switch egressResourceType {
	case ComputeVPNGateway:
...
func (r *NetworkEgressUsage) getEgressAPIRegionName(region string, egressResourceType EgressResourceType) *string {
	switch egressResourceType {
	case ComputeExternalVPNGateway, ComputeVPNGateway:
...
func (r *NetworkEgressUsage) getEgressAPIServiceName(egressResourceType EgressResourceType) *string {
	switch egressResourceType {
	case ComputeExternalVPNGateway, ComputeVPNGateway:
