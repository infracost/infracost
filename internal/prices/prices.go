...
func setCostComponentPrice(ctx *config.RunContext, currency string, r *schema.Resource, c *schema.CostComponent, res gjson.Result) {
	...
	if c.CustomPrice() != nil {
		logging.Logger.Debug().Msgf("Using user-defined custom price %v for %s %s.", *c.CustomPrice(), r.Name, c.Name)
	...
	if len(products) == 0 {
		if c.IgnoreIfMissingPrice {
			logging.Logger.Debug().Msgf("No products found for %s %s, ignoring since IgnoreIfMissingPrice is set.", r.Name, c.Name)
			r.RemoveCostComponent(c)
			return
		}

		logging.Logger.Warn().Msgf("No products found for %s %s, using 0.00", r.Name, c.Name)
	...
	if len(products) > 1 {
		logging.Logger.Debug().Msgf("Multiple products found for %s %s, filtering those with prices", r.Name, c.Name)
	}
	...
	if len(productsWithPrices) == 0 {
		if c.IgnoreIfMissingPrice {
			logging.Logger.Debug().Msgf("No prices found for %s %s, ignoring since IgnoreIfMissingPrice is set.", r.Name, c.Name)
			r.RemoveCostComponent(c)
			return
		}

		logging.Logger.Warn().Msgf("No prices found for %s %s, using 0.00", r.Name, c.Name)
	...
	if len(productsWithPrices) > 1 {
		logging.Logger.Debug().Msgf("Multiple products with prices found for %s %s, using the first product", r.Name, c.Name)
		setResourceWarningEvent(ctx, r, "Multiple products found")
	}

	prices := productsWithPrices[0].Get("prices").Array()
	if len(prices) > 1 {
		logging.Logger.Warn().Msgf("Multiple prices found for %s %s, using the first price", r.Name, c.Name)
