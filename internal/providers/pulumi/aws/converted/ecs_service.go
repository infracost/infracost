...
func getECSServiceRegistryItem() *schema.RegistryItem {
	return &schema.RegistryItem{
		...
		RFunc:           NewECSService,
		ReferenceAttributes: []string{"cluster", "taskDefinition", "awsEcsTaskSet.service"},
	}
}
...
func NewECSService(d *schema.ResourceData, u *schema.UsageData) *schema.Resource {
	...
	for _, ref := range d.References("taskDefinition") {
	...
	for _, ref := range d.References("awsEcsTaskSet.service") {
	...
	r := &aws.ECSService{
		...
		DesiredCount:                   d.Get("desiredCount").Int(),
		MemoryGB:                       memoryGB,
		VCPU:                           vcpu,
...
func calcLaunchType(d *schema.ResourceData) string {
	...
	launchType := d.Get("launchType").String()
	if launchType != "" {
		return launchType
	}

	// Check for an active direct capacity provider
	launchType = getCapacityProviderLaunchType(d.Get("capacityProviderStrategy").Array())
	if launchType != "" {
		return launchType
	}
