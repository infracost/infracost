...
func getContainerClusterRegistryItem() *schema.RegistryItem {
	return &schema.RegistryItem{
		...
		ReferenceAttributes: []string{"googleContainerNodePool.cluster"},
...
func newContainerCluster(d *schema.ResourceData) schema.CoreResource {
	...
	autopilotEnabled := d.Get("enableAutopilot").Bool()
	...
	for _, ref := range d.References("googleContainerNodePool.cluster") {
		definedNodePoolNames = append(definedNodePoolNames, ref.Get("name").String())
	}

	if !autopilotEnabled {
		nameIndex := 0
		for _, values := range d.Get("nodePool").Array() {
		...
		if defaultNodePool == nil && !d.Get("removeDefaultNodePool").Bool() {
			...
			defaultNodePool = &google.ContainerNodePool{
				...
				NodeConfig:   newContainerNodeConfig(d.Get("nodeConfig.0")),
			}
		}
	}
...
func zoneCount(d gjson.Result, location string) int {
	...
	if len(d.Get("nodeLocations").Array()) > 0 {
		c = len(d.Get("nodeLocations").Array())
	}
...
func newContainerNodeConfig(d gjson.Result) *google.ContainerNodeConfig {
	...
	if d.Get("diskType").Exists() {
		diskType = d.Get("diskType").String()
	}

	diskSize := int64(100)
	if d.Get("diskSizeGb").Exists() {
		diskSize = d.Get("diskSizeGb").Int()
	}

	localSSDCount := d.Get("localSsdCount").Int()

	guestAccelerators := collectComputeGuestAccelerators(d.Get("guestAccelerator"))
