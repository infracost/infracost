...
func computeImageDiskSize(d *schema.ResourceData) float64 {
	if d.Get("diskSizeGb").Exists() {
		return d.Get("diskSizeGb").Float()
	}

	if len(d.References("sourceDisk")) > 0 {
		return computeDiskSize(d.References("sourceDisk")[0])
	}

	if len(d.References("sourceImage")) > 0 {
		return computeImageDiskSize(d.References("sourceImage")[0])
	}

	if len(d.References("sourceSnapshot")) > 0 {
		return computeSnapshotDiskSize(d.References("sourceSnapshot")[0])
	}
...
func computeIOPS(d *schema.ResourceData, diskType string, diskSize float64) int64 {
	if diskType == "pd-extreme" || diskType == "hyperdisk-extreme" {
		...
		if d.Get("provisionedIops").Exists() {
			return d.Get("provisionedIops").Int()
		}
