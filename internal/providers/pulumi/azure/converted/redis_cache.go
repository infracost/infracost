...
func NewAzureRMRedisCache(d *schema.ResourceData, u *schema.UsageData) *schema.Resource {
	...
	if strings.EqualFold(skuName, "premium") {
		...
		if d.Get("shardCount").Int() > 0 {
			shards = d.Get("shardCount").Int()
		}

		if d.Get("replicasPerPrimary").Type != gjson.Null {
			nodesPerShard = 1 + d.Get("replicasPerPrimary").Int()
		} else if d.Get("replicasPerMaster").Type != gjson.Null {
			nodesPerShard = 1 + d.Get("replicasPerMaster").Int()
		}
	}
