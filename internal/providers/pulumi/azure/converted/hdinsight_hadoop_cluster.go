...
func NewAzureRMHDInsightHadoopCluster(d *schema.ResourceData, u *schema.UsageData) *schema.Resource {
	...
	if d.Get("roles.0.edgeNode.0.vmSize").Type != gjson.Null {
		edgeNodeVM = d.Get("roles.0.edgeNode.0.vmSize").String()
		var workerInstances int64
		if d.Get("roles.0.edgeNode.0.targetInstanceCount").Type != gjson.Null {
			workerInstances = d.Get("roles.0.edgeNode.0.targetInstanceCount").Int()
		}
