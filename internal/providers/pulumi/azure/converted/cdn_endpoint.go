...
func NewAzureRMCDNEndpoint(d *schema.ResourceData, u *schema.UsageData) *schema.Resource {
	...
	if len(d.References("profileName")) > 0 {
		profile = d.References("profileName")[0]
	...
	if strings.ToLower(sku) == "standard_microsoft" {
		numberOfRules := 0
		if d.Get("globalDeliveryRule").Type != gjson.Null {
			numberOfRules += len(d.Get("globalDeliveryRule").Array())
		}
		if d.Get("deliveryRule").Type != gjson.Null {
			numberOfRules += len(d.Get("deliveryRule").Array())
		}
	...
	if strings.ToLower(sku) == "standard_akamai" || strings.ToLower(sku) == "standard_verizon" {
		if d.Get("optimizationType").Type != gjson.Null {
			if strings.ToLower(d.Get("optimizationType").String()) == "dynamicsiteacceleration" {
