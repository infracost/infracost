...
func NewAzureRMSearchService(d *schema.ResourceData, u *schema.UsageData) *schema.Resource {
	...
	if d.Get("partitionCount").Type != gjson.Null {
		partitionCount = decimal.NewFromInt(d.Get("partitionCount").Int())
	}
	if d.Get("replicaCount").Type != gjson.Null {
		replicaCount = decimal.NewFromInt(d.Get("replicaCount").Int())
	}
