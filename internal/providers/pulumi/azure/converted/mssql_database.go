...
func newAzureRMMSSQLDatabase(d *schema.ResourceData) schema.CoreResource {
	...
	sku := d.GetStringOrDefault("skuName", "GP_S_Gen5_2")

	var maxSize *float64
	if !d.IsEmpty("max_size_gb") {
		val := d.Get("maxSizeGb").Float()
		maxSize = &val
	}

	var replicaCount *int64
	if !d.IsEmpty("read_replica_count") {
		val := d.Get("readReplicaCount").Int()
		replicaCount = &val
	}

	licenseType := d.GetStringOrDefault("licenseType", "LicenseIncluded")
	storageAccountType := d.GetStringOrDefault("storageAccountType", "Geo")
	...
	r := &azure.SQLDatabase{
		...
		ZoneRedundant:     d.Get("zoneRedundant").Bool(),
