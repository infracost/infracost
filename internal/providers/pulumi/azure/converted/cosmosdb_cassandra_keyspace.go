...
func GetAzureRMCosmosdbCassandraKeyspaceRegistryItem() *schema.RegistryItem {
	return &schema.RegistryItem{
		...
		GetRegion: func(defaultRegion string, d *schema.ResourceData) string {
			if len(d.References("accountName")) > 0 {
				account := d.References("accountName")[0]
...
func cosmosDBCostComponents(d *schema.ResourceData, u *schema.UsageData, account *schema.ResourceData) []*schema.CostComponent {
	...
	} else if d.Get("autoscaleSettings.0.maxThroughput").Type != gjson.Null {
		throughputs = decimalPtr(decimal.NewFromInt(d.Get("autoscaleSettings.0.maxThroughput").Int()))
		model = Autoscale
	} else {
		model = Serverless
