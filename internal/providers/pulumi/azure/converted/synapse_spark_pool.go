...
func NewAzureRMSynapseSparkPool(d *schema.ResourceData, u *schema.UsageData) *schema.Resource {
	...
	nodeSize := "Small"
	if d.Get("nodeSize").Type != gjson.Null {
		nodeSize = d.Get("nodeSize").String()
	}
	...
	if d.Get("nodeCount").Type != gjson.Null {
		nodeCount = decimalPtr(decimal.NewFromInt(d.Get("nodeCount").Int()))
	}

	if nodeCount == nil {
		if d.Get("autoScale").Type != gjson.Null {
			autoScale := d.Get("autoScale").Array()
			if len(autoScale) > 0 {
