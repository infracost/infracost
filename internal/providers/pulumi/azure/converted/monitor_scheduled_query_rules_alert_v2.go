...
func getMonitorScheduledQueryRulesAlertV2RegistryItem() *schema.RegistryItem {
	return &schema.RegistryItem{
		...
		RFunc: newMonitorScheduledQueryRulesAlertV2,
...
func newMonitorScheduledQueryRulesAlertV2(d *schema.ResourceData) schema.CoreResource {
	...
	freq := int64(1)
	ef, err := duration.FromString(d.Get("evaluationFrequency").String())
	if err != nil {
		logging.Logger.Warn().Str(
			"resource", d.Address,
		).Msgf("failed to parse ISO8061 duration string '%s' using 1 minute frequency", d.Get("evaluationFrequency").String())
	} else {
