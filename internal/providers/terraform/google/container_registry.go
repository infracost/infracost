...
func NewContainerRegistry(d *schema.ResourceData, u *schema.UsageData) *schema.Resource {
	r := &google.ContainerRegistry{Address: d.Address, Region: d.Get("region").String(), Location: d.Get("location").String()}
	if !d.IsEmpty("storage_class") {
		r.StorageClass = d.Get("storage_class").String()
	}
	r.PopulateUsage(u)
