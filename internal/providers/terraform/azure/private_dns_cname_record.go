...
func NewAzureRMPrivateDNScnameRecord(d *schema.ResourceData, u *schema.UsageData) *schema.Resource {
	...
	if u != nil && u.Get("monthly_queries").Type != gjson.Null {
		...
		if monthlyQueries.GreaterThan(*firstOneBQueries) {
			...
			costComponents = append(costComponents, PrivateDNScnameQueriesOverCostComponent(location, monthlyQueries))
		}
	}

	costComponents = append(costComponents, PrivateDNScnameQueriesFirstCostComponent(location, firstOneBQueries))
	return &schema.Resource{
		Name:           d.Address,
		CostComponents: costComponents,
	}
}
func PrivateDNScnameQueriesFirstCostComponent(location string, monthlyQueries *decimal.Decimal) *schema.CostComponent {
...
func PrivateDNScnameQueriesOverCostComponent(location string, monthlyQueries *decimal.Decimal) *schema.CostComponent {
	return &schema.CostComponent{
