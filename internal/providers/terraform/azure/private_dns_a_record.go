...
func NewAzureRMPrivateDNSaRecord(d *schema.ResourceData, u *schema.UsageData) *schema.Resource {
	...
	if u != nil && u.Get("monthly_queries").Type != gjson.Null {
		...
		if monthlyQueries.GreaterThan(*firstOneBQueries) {
			...
			costComponents = append(costComponents, PrivateDNSqueriesOverCostComponent(location, monthlyQueries))
		}
	}

	costComponents = append(costComponents, PrivateDNSqueriesFirstCostComponent(location, firstOneBQueries))
	return &schema.Resource{
		Name:           d.Address,
		CostComponents: costComponents,
	}
}
func PrivateDNSqueriesFirstCostComponent(location string, monthlyQueries *decimal.Decimal) *schema.CostComponent {
...
func PrivateDNSqueriesOverCostComponent(location string, monthlyQueries *decimal.Decimal) *schema.CostComponent {
	return &schema.CostComponent{
