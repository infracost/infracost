...
func NewAzureRMPrivateDNSsrvRecord(d *schema.ResourceData, u *schema.UsageData) *schema.Resource {
	...
	if u != nil && u.Get("monthly_queries").Type != gjson.Null {
		...
		if monthlyQueries.GreaterThan(*firstOneBQueries) {
			...
			costComponents = append(costComponents, PrivateDNSsrvQueriesOverCostComponent(location, monthlyQueries))
		}
	}

	costComponents = append(costComponents, PrivateDNSsrvQueriesFirstCostComponent(location, firstOneBQueries))
	return &schema.Resource{
		Name:           d.Address,
		CostComponents: costComponents,
	}
}
func PrivateDNSsrvQueriesFirstCostComponent(location string, monthlyQueries *decimal.Decimal) *schema.CostComponent {
...
func PrivateDNSsrvQueriesOverCostComponent(location string, monthlyQueries *decimal.Decimal) *schema.CostComponent {
	return &schema.CostComponent{
