...
func NewAzureRMDNScnameRecord(d *schema.ResourceData, u *schema.UsageData) *schema.Resource {
	...
	if u != nil && u.Get("monthly_queries").Type != gjson.Null {
		...
		if monthlyQueries.GreaterThan(*firstOneBQueries) {
			...
			costComponents = append(costComponents, DNScnameQueriesOverCostComponent(location, monthlyQueries))
		}
	}

	costComponents = append(costComponents, DNScnameQueriesFirstCostComponent(location, firstOneBQueries))
	return &schema.Resource{
		Name:           d.Address,
		CostComponents: costComponents,
	}
}
func DNScnameQueriesFirstCostComponent(location string, monthlyQueries *decimal.Decimal) *schema.CostComponent {
...
func DNScnameQueriesOverCostComponent(location string, monthlyQueries *decimal.Decimal) *schema.CostComponent {
	return &schema.CostComponent{
