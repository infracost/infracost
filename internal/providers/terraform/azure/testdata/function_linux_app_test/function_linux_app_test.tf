...
resource "azurerm_linux_function_app" "function_with_usage" {
  for_each = { for entry in azurerm_service_plan.plan : "${entry.name}" => entry }
...
resource "azurerm_linux_function_app" "legacy_service_plan_function" {
...
resource "azurerm_linux_function_app" "legacy_service_plan_function_with_usage" {
  for_each = { for entry in azurerm_app_service_plan.legacy_plan : "${entry.name}" => entry }
