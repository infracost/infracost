...
func NewAzureRMPrivateDNSptrRecord(d *schema.ResourceData, u *schema.UsageData) *schema.Resource {
	...
	if u != nil && u.Get("monthly_queries").Type != gjson.Null {
		...
		if monthlyQueries.GreaterThan(*firstOneBQueries) {
			...
			costComponents = append(costComponents, PrivateDNSptrQueriesOverCostComponent(location, monthlyQueries))
		}
	}

	costComponents = append(costComponents, PrivateDNSptrQueriesFirstCostComponent(location, firstOneBQueries))
	return &schema.Resource{
		Name:           d.Address,
		CostComponents: costComponents,
	}
}
func PrivateDNSptrQueriesFirstCostComponent(location string, monthlyQueries *decimal.Decimal) *schema.CostComponent {
...
func PrivateDNSptrQueriesOverCostComponent(location string, monthlyQueries *decimal.Decimal) *schema.CostComponent {
	return &schema.CostComponent{
