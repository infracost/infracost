...
func NewEC2TransitGatewayVpcAttachment(d *schema.ResourceData, u *schema.ResourceData) *schema.Resource {
	...
	return &schema.Resource{
		Name: d.Address,
		CostComponents: transitGatewayCostComponent(region, "TransitGatewayVPC", gbDataProcessed),
	}
}

func transitGatewayCostComponent(region string, operation string, gbDataProcessed *decimal.Decimal) []*schema.CostComponent {
	...
	if operation != "TransitGatewayPeering" {
