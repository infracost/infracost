		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if ctx.IsCloudUploadExplicitlyEnabled() {
				if ctx.Config.IsSelfHosted() {
					...
					result := shareCombinedRun(ctx, combined, inputs, false)
...
func shareCombinedRun(ctx *config.RunContext, combined output.Root, inputs []output.ReportInput, commentBasicSyntax bool) apiclient.AddRunResponse {
	...
	result, err := dashboardClient.AddRun(ctx, combined, commentBasicSyntax)
	if err != nil {
